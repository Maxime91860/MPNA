...
double *vecteur2_complet=(double *)malloc(nb_lignes*sizeof(double));
double *matrice_complete=(double *)malloc((nb_lignes*nb_colonnes)*sizeof(double));
}
...
for(i=0; i<nb_lignes; i++){
	vecteur1_complet[i]=2;
	vecteur2_complet[i]=2;

	for(j=0; j<nb_colonnes; j++){
	matrice_complet[j+(i*nb_colonnes)]=2;		
	}
}
